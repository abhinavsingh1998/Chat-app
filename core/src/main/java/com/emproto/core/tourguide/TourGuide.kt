package com.emproto.core.tourguide
...
import com.emproto.core.tourguide.util.locationOnScreen
...
open class TourGuide(private val activity: Activity) {
    ...
    lateinit var toolTipContainer: LinearLayout
    lateinit var toolTipTitleTextView:TextView
    lateinit var toolTipDescTextView:TextView
    ...
    private fun handleDisableClicking(frameLayoutWithHole: FrameLayoutWithHole) {
        overlay?.also { _overlay ->
            if (_overlay.mOnClickListener != null) {
                ...
                Log.w(
                    "tourguide",
                    "Overlay's default OnClickListener is null, it will proceed to next tourguide when it is clicked"
                )
    ...
    private fun setupToolTip() {
        ...
        toolTip?.also { _toolTip ->
            ...
            if (_toolTip.getCustomView() == null) {
                ...
                toolTipView?.apply {
                    toolTipContainer = toolTipView!!.findViewById(R.id.toolTipContainer)
                    toolTipTitleTextView = toolTipContainer.findViewById(R.id.toolTipTitleTextView)
                    toolTipDescTextView = toolTipContainer.findViewById(R.id.toolTipDescTextView)
            ...
            toolTipView?.also { _toolTipView ->
                ...
                _toolTipView.measure(
                    FrameLayout.LayoutParams.WRAP_CONTENT,
                    FrameLayout.LayoutParams.WRAP_CONTENT
                )
                var toolTipMeasuredWidth =
                    if (_toolTip.mWidth != -1) _toolTip.mWidth else _toolTipView.measuredWidth
                ...
                val adjustment =
                    10 * density //adjustment is that little overlapping area of tourguide_tooltip and targeted button
                ...
                if (toolTipMeasuredWidth > parent.width) {
                    resultPoint.x =
                        getXForTooTip(_toolTip.mGravity, parent.width, targetViewX, adjustment)
                } else {
                    resultPoint.x = getXForTooTip(
                        _toolTip.mGravity,
                ...
                resultPoint.y =
                    getYForTooTip(_toolTip.mGravity, toolTipMeasuredHeight, targetViewY, adjustment)
                ...
                if (resultPoint.x < 0) {
                    _toolTipView.layoutParams.width =
                        toolTipMeasuredWidth + resultPoint.x //since point.x is negative, use plus
                    resultPoint.x = 0
                }
                ...
                if (tempRightX > parent.width) {
                    _toolTipView.layoutParams.width =
                        parent.width - resultPoint.x //since point.x is negative, use plus
                }
                ...
                _toolTipView.viewTreeObserver.addOnGlobalLayoutListener(object :
                    ViewTreeObserver.OnGlobalLayoutListener {
                    override fun onGlobalLayout() {
                        ...
                        fixedY = getYForTooTip(
                            _toolTip.mGravity,
                            toolTipHeightAfterLayouted,
                            targetViewY,
                            adjustment
                        )
    ...
    private fun getXForTooTip(
        gravity: Int,
        toolTipMeasuredWidth: Int,
        targetViewX: Int,
        adjustment: Float
    ) =
        when {
    ...
    private fun getYForTooTip(
        gravity: Int,
        toolTipMeasuredHeight: Int,
        targetViewY: Int,
        adjustment: Float
    ) =
        when {
    ...
    private fun setupAndAddFABToFrameLayout(frameLayoutWithHole: FrameLayoutWithHole): FloatingActionButton {
        ...
        return FloatingActionButton(activity).apply {
            ...
            invisFab.viewTreeObserver.addOnGlobalLayoutListener(object :
                ViewTreeObserver.OnGlobalLayoutListener {
                override fun onGlobalLayout() {
                    // make sure this only run once
                    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
                    ...
                    params.setMargins(
                        getXBasedOnGravity(invisFab.width),
                        getYBasedOnGravity(invisFab.height),
                        0,
                        0
                    )
                }
